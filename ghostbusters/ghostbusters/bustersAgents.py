...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        closestGhostPos = self.findClosestGhostPosition(ghostPositions, pacmanPosition)
        bestAction = self.getBestAction(closestGhostPos, legal, pacmanPosition)
        return bestAction

    def getBestAction(self, closestGhostPos, legal, pacmanPosition):
        bestAction = legal[0]
        bestDistance = float('inf')
        for action in legal:
            sucPos = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(sucPos, closestGhostPos)
            if distance < bestDistance:
                bestDistance = distance
                bestAction = action
        return bestAction

    def findClosestGhostPosition(self, ghostPositions, pacmanPosition):
        closestGhost = None
